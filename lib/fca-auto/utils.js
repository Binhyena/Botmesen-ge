    ...
    return function(/** @type {{ headers: { [x: string]: any[]; }; }} */res) {
        ...
        cookies.forEach(function(/** @type {string} */c) {
            if (c.indexOf("facebook.com") > -1) { // yo wtf is this?
                try { jar.setCookie(c, "https://www.facebook.com"); } catch(e) {}
                try { jar.setCookie(c.replace(/domain=(?:www\.)?\.?facebook\.com/i, "domain=.messenger.com"), "https://www.messenger.com"); } catch(e) {}
            }
        });
        return res;
    };
